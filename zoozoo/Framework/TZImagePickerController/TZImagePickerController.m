...
- (void)viewDidLoad {
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = [UIColor colorWithRed:135/255.0 green:73/255.0 blue:233/255.0 alpha:1.0];
    self.oKButtonTitleColorDisabled = [UIColor colorWithRed:135/255.0 green:73/255.0 blue:233/255.0 alpha:1.0];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        self.videoMaximumDuration = 1 * 30;
...
- (void)configDefaultImageName {
    ...
    self.photoSelImageName = @"quanjuxuanzhong";
    self.photoDefImageName = @"selectdian";
    ...
    self.photoOriginSelImage = [self createImageWithColor:nil size:CGSizeMake(20, 20) radius:10];
}
...
- (void)viewDidLoad {
    ...
    [btn addTarget:imagePickerVc action:@selector(cancelButtonClick) forControlEvents:UIControlEventTouchUpInside];
